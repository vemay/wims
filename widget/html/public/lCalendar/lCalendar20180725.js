...
window.lCalendar = (function() {
	...
	MobileCalendar.prototype = {
		init: function(params) {
			...
			this.frameBounds = params.frameBounds || false; //记录apicloud是否设置了页面弹动
			this.callBack = params.callBack || function () {}; //设置确认日期时间后回调
    ...
    setDefaultValue:function (value) {
      ...
      function popupSelect(e) {
        ...
        selectCtrlInit(_self.defaultValue);
      ...
      //初始化select 选项默认值
      function selectCtrlInit(defaultValue) {
        // var date;
        // if (defaultValue) {
        ...
        // }
				var date;
        _self.gearDate.querySelector(".date_yy").setAttribute("val", _self.selectArr);
        setDateGearTooth();
      }
			/*extend end*/
			//呼出日期插件
			function popupDate(e) {
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
					...
					'</div>';
				document.body.appendChild(_self.gearDate);
				dateCtrlInit(_self.defaultValue);
				var lcalendar_cancel = _self.gearDate.querySelector(".lcalendar_cancel");
				lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
				var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
				lcalendar_finish.addEventListener('touchstart', finishMobileDate);
			...
			//初始化年月日插件默认值
			function dateCtrlInit(defaultValue) {
        var date;
        if (defaultValue) {
          date = new Date(defaultValue);
        } else {
          date = new Date();
			...
			function popupDateTime(e) {
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
					...
					'</div>';
				document.body.appendChild(_self.gearDate);
				dateTimeCtrlInit(_self.defaultValue);
				var lcalendar_cancel = _self.gearDate.querySelector(".lcalendar_cancel");
				lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
				var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
			...
			//初始化年月日时分插件默认值
			function dateTimeCtrlInit(defaultValue) {
        var date;
        if (defaultValue) {
          date = new Date(defaultValue);
        } else {
          date = new Date();
			...
			function popupTime(e) {
				...
				_self.gearDate.innerHTML = '<div class="date_ctrl slideInUp">' +
					...
					'</div>';
				document.body.appendChild(_self.gearDate);
				timeCtrlInit(_self.defaultValue);
				var lcalendar_cancel = _self.gearDate.querySelector(".lcalendar_cancel");
				lcalendar_cancel.addEventListener('touchstart', closeMobileCalendar);
				var lcalendar_finish = _self.gearDate.querySelector(".lcalendar_finish");
			...
			//初始化时分插件默认值
			function timeCtrlInit(defaultValue) {
        var date;
        if (defaultValue) {
          date = new Date(defaultValue);
        } else {
          date = new Date();
			...
			function gearTouchStart(e) {
				_self.frameBounds && frameSetNobounces(); //关闭apicloud页面弹动 wwg
				e.preventDefault();
				var target = e.target;
        while (true) {
			...
			function gearTouchEnd(e) {
				_self.frameBounds && frameSetbounces(); //如果apicloud设置了弹动 恢复之
				e.preventDefault();
				var target = e.target;
				while (true) {
					if (!target.classList.contains("gear")) {
