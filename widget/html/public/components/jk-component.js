...
Vue.component('jk-more-list',{
  ...
  '<div class="more-mask" v-show="moreShow" @click="closeMask">' +
              ' <transition name="more" >'+
                '<div class="handle-list-common" v-show="moreShow">'+
                  ...
                  '<div class="handle-item click-effect" v-if="!more.hide" v-for="(more,index) in moreList" @click="doHandle(more,index)">'+
  ...
  methods:{
    ...
    doHandle:function (item,index) {
      this.$emit('listclick',item,index);
      this.$emit(item.fn,item,index)
    },
  }
});
/*右上角更多列表结束*/
...
Vue.component('jk-item-pages',{
  ...
  '                       v-for="(page,index) in pages"\n' +
  '                       @click.prevent="handlePage(page.index)"\n' +
  '                >\n' +
  '                    <input type="checkbox" v-model="selectPages" :value="page">\n' +
  '                    <span class="text"\n' +
  '                          :class="{\'current-page\':currentPageIndex===page.index}"\n' +
  '                    >{{page.text}}</span>\n' +
  '                </label>\n' +
  '            </div>\n' +
  ...
  computed:{
    pages:function() {
      var arr = [];
      var i = 1;
      if (this.list.length>0) { //处理表体数据 非删除状态的条目重组成展示索引
        this.list.forEach(function(val,index) {
          if (val.sEditFlag !== 'D') {
            arr.push({text:i+'',index:index});
            i++;
          }
        })
      }
    ...
    realCurrentPageIndex:function () {
      //实际选中的元素索引(数组中含删除状态元素)
      var _this = this;
      var realCurrentPageIndex = -1;
      this.pages.forEach(function (value,index) {
        if (value.index === _this.currentPageIndex) {
          realCurrentPageIndex = index;
        }
      });
      return realCurrentPageIndex;
    }
  },
  watch:{
  ...
  methods:{
    ...
    selectCurrent:function () {
      var i = this.selectPages.indexOf(this.pages[this.realCurrentPageIndex]);
      if (i < 0) {
        this.selectPages.push(this.pages[this.realCurrentPageIndex]);
      } else {
    ...
    nextPage:function() {
      if (this.realCurrentPageIndex < this.pages.length - 1) {
        this.handlePage( this.pages[this.realCurrentPageIndex+1].index )
      } else {
        return false;
      }
    ...
    prevPage:function () {
      if (this.realCurrentPageIndex > 0) {
        this.handlePage( this.pages[this.realCurrentPageIndex-1].index)
      } else {
        return false;
      }
...
'                  </select>' +
'                </div>\n' +
'              <label class="con check-label" v-if="entryType===\'checkbox\'">\n' +
'                  <input type="checkbox" class="input-border-primary entry-checkbox" \n' +
'                      :value="value"' +
...
'               </label>' +
'               <div class="con" v-if="entryType===\'radio\'" >\n' +
'                  <label class="radio-label" v-for="radio in radioArr">\n' +
'                     <input type="radio" ' +
'                             :name="radioName"' +
'                             :value="radio[radioValueAttr]"' +
'                             v-on:change="$emit(\'change\',$event.target.checked)"' +
'                      >\n' +
'                     <span class="radio-text">' +
'                      <i class="icon iconfont jk-true"></i>' +
'                      {{radio[radioStrAttr]}}</span>\n' +
'                  </label>\n' +
'               </div>' +
'               <div class="con m-check" v-if="entryType===\'mcheck\'">\n' +
'                  <label class="radio-label" v-for="(mCheck,index) in mCheckDataArr">\n' +
'                     <input type="checkbox" ' +
'                             :name="mCheckName"' +
'                             :value="mCheck[mCheckValueAttr]"' +
'                             :checked="mCheck[\'isChecked\']"' +
'                             :disabled="disabled"' +
'                             v-on:change="getMCheckValue">\n' +
'                     <span class="radio-text">' +
'                      <i class="icon iconfont jk-true"></i>' +
'                      <span>{{mCheck[mCheckStrAttr]}}</span></span>\n' +
'                  </label>' +
'               </div>' +
'               <div class="con" v-if="entryType===\'date\'">\n' +
'                  <input :id="dateId" type="text" class="input-border-primary"' +
'                      :date-type="dateType"' +
'                      v-on:focus="dateFocus" '+
...
'         </div>',
  props:{
    ...
    opValueAttr:{default:'sEnumCode'}, //枚举选择项值字段
    opStrAttr:{default:'sEnumValue'}, //枚举选择项文本字段
    radioArr:{
      default:function () {
        return [];
      }},
    
    radioValueAttr:{default:'sEnumCode'}, //单选选择项值字段
    radioStrAttr:{default:'sEnumValue'}, //单选选择项文本字段
    mCheckArr:{
      default:function () {
        return [];
      }},
    mCheckValueAttr:{default:'sEnumCode'}, //多选选择项值字段
    mCheckStrAttr:{default:'sEnumValue'}, //多选选择项文本字段
    /*pick-data*/
  ...
  watch:{
    value:{
      handler:function () {
        ...
        } else if (this.entryType === 'mcheck') {
          this.mCheckDataArr = this.mCheckArr;
          this.setMCheckValue();
        }
      },
      immediate:true
  ...
  data:function(){
    return {
      calendar:null,
      dateId:'id'+Math.ceil(Math.random()*1000),
      radioName:'name'+Math.ceil(Math.random()*1000),
      mCheckName:'name'+Math.ceil(Math.random()*1000),
      mCheckDataArr:[]
    }
  },
  mounted:function() {
  ...
  methods:{
    getMCheckValue:function (e) {
      ...
      if (e.target.checked) {
        this.mCheckDataArr.forEach(function (value,index) {
          if (value[t.mCheckValueAttr]+'' === e.target.value+'') {
            value.isChecked = true;
          }
          mCheckValue.push(value);
        })
      } else {
        this.mCheckDataArr.forEach(function (value,index) {
          if (value[t.mCheckValueAttr]+'' === e.target.value+'') {
      ...
      mCheckValue.forEach(function (value) {
        if (value.isChecked) {
          textArr.push(value[t.mCheckValueAttr]);
        }
      });
      this.$emit('change',textArr.join('|'));
    },
    setMCheckValue:function () {
      //初始化多选
      var t = this;
      this.mCheckDataArr = this.mCheckDataArr.filter(function (value,index) {
          return value[t.mCheckValueAttr] !== '';
      });
      this.mCheckDataArr.map(function(val,index){
        if (t.value.split('|').indexOf(val[t.mCheckValueAttr])>=0) {
          val.isChecked = true;
        }
        return val;
      });
    ...
    dateFocus:function (e) {
      this.calendar.setDefaultValue(e.target.value);
    },
    dateBlur:function (e) {
      this.$emit('date-blur',this.value);
    },
...
Vue.component('jk-load-more',
  {
    ...
    props: {
      ...
      topDropText: {type: String, default: "松开刷新"},
      ...
      bottomDropText: {type: String, default: "松开加载"},
...
Vue.component('jk-date-input',{
  template:'<input :id="dateId" class="date-input-style"\n' +
  '                        type="text"\n' +
  '                        data-lcalendar="2000-01-1,2099-12-31"\n' +
  '                        :date-type="dateType"' +
  '                        :frame-bounds="frameBounds"' +
  ...
  props:{
    value:{default:''},
    dateType:{default:'date_time'},
    frameBounds:{default:false},
    placeholder:{default:'日期时间'},
  },
  data:function () {
    return {
      dateId:'id'+Math.ceil(Math.random()*10000)
    }
  },
  mounted:function () {
    this.initCalendar();
...
/*jk-date-input end*/
/*jk-head-page*/
Vue.component('jk-head-page',{
  template:'<div class="jk-head-page">\n' +
  '        <span class="page-btn "\n' +
  '              :class="{disabled:bofBoolean,\'click-effect\':!bofBoolean}"\n' +
  '              @click="billChange(firstPage)"\n' +
  '        >首</span>\n' +
  '        <span class="page-btn "\n' +
  '              :class="{disabled:bofBoolean,\'click-effect\':!bofBoolean}"\n' +
  '              @click="billChange(prePage)"\n' +
  '        >上</span>\n' +
  '        <span class="page-btn "\n' +
  '              :class="{disabled:eofBoolean,\'click-effect\':!eofBoolean}"\n' +
  '              @click="billChange(nextPage)"\n' +
  '        >下</span>\n' +
  '        <span class="page-btn "\n' +
  '              :class="{disabled:eofBoolean,\'click-effect\':!eofBoolean}"\n' +
  '              @click="billChange(lastPage)"\n' +
  '        >尾</span>\n' +
  '      </div>',
  props:{
    bof:{default:false},
    eof:{default:false},
    firstPage:{default:'FirstPage'},
    prePage:{default:'PrePage'},
    nextPage:{default:'NextPage'},
    lastPage:{default:'LastPage'}
  },
  computed:{
    bofBoolean:function () {
      var ret = undefined;
      if (typeof this.bof === 'string') {
        if (this.bof === 'True') {
          ret = true;
        } else if (this.bof === 'False') {
          ret = false;
        }
      }
      return Boolean(ret);
    ...
    eofBoolean:function () {
      var ret = undefined;
      if (typeof this.eof === 'string') {
        if (this.eof === 'True') {
          ret = true;
        } else if (this.eof === 'False') {
          ret = false;
        }
      }
      return Boolean(ret);
  ...
  methods:{
    billChange:function (pageType) {
      //pageType值 FirstPage PrePage NextPage LastPage
      if ( this.bofBoolean) {
        if (pageType === this.prePage || pageType === this.firstPage) {
          return false;
        }
      }
      if (this.eofBoolean) {
        if (pageType === this.nextPage || pageType === this.lastPage) {
          return false;
        }
      }
      this.$emit('bill-change',pageType);
    }
  }
})
/*jk-head-page end*/